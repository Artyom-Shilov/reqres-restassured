...
public abstract class BaseRequestsParametersPropertyReader {
    ...
    protected int calculateCaseAmount(String keyPhrase, Properties properties) {
        return (int) properties.entrySet().stream()
                .filter(e -> ((String)e.getKey()).contains(keyPhrase))
                .count();
    }

    protected List<String> getListOfTestData(String keyPhrase, Properties properties) {
    ...
    private Object[] formArrayFromOneCaseData(String data) {
        if (data.contains(",")) {
    ...
    protected Object[][] getDataForTestNgDataProvider(String keyPhrase, Properties properties) {
